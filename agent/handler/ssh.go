...
func (mySSH *SSH) start() {
	...
	sshDial, err := ssh.Dial("tcp", mySSH.Addr, &ssh.ClientConfig{
		...
		HostKeyCallback: func(hostname string, remote net.Addr, key ssh.PublicKey) error {
			...
			if mySSH.HostKeyFingerprint != "" {
				if fp != mySSH.HostKeyFingerprint {
					return fmt.Errorf("SSH host key mismatch: got %s, expected %s", fp, mySSH.HostKeyFingerprint)
				}
...
func DispatchSSHMess(mgr *manager.Manager) {
	...
	for {
		...
		case *protocol.SSHReq:
			...
			mySSH.HostKeyFingerprint = mess.HostKeyFingerprint
			go mySSH.start()
