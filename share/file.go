...
func sanitizePath(path string) (string, error) {
	...
	if !strings.HasPrefix(realDir+string(filepath.Separator), realCwd+string(filepath.Separator)) && realDir != realCwd {
...
func (file *MyFile) SendFileStat(route string, targetUUID string, identity int) error {
	...
	cleanPath, err := sanitizePath(file.FilePath)
	if err != nil {
		return err
	}

	fileHandler, err := os.Open(cleanPath)
	if err != nil {
		return err
	}
...
func (file *MyFile) CheckFileStat(route string, targetUUID string, identity int) error {
	...
	cleanName, err := sanitizePath(file.FileName)
	if err != nil {
		return err
	}

	fileHandler, err := os.OpenFile(cleanName, os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0600)
	if err != nil {
		return err
	}
