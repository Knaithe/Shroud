...
# ================================================================
docker run -d --name s-a --network s-net --entrypoint /opt/shroud/admin shroud-test \
  --daemon -l 9999 -s "$SECRET" --identity-plain --identity-dir /tmp 2>/dev/null; sleep 3
...
# ================================================================
SC=$(timeout 15 $A -l 19998 -s sc_test --identity-plain --identity-dir /tmp/sc --script 2>&1 <<< "help" || true)
...
# ================================================================
docker run -d --name s-a --network s-net --entrypoint /opt/shroud/admin shroud-test \
  --daemon -l 9999 -s "$SECRET" --identity-plain --identity-dir /tmp --heartbeat 2>/dev/null; sleep 3
docker run -d --name s-g --network s-net --entrypoint /opt/shroud/agent shroud-test \
...
# Kill admin and wait for watchdog
docker kill s-a 2>/dev/null
echo "Waiting 100s for agent watchdog..."
sleep 100

docker ps --format '{{.Names}}' 2>/dev/null | grep -q s-g \
  && fail "7.1" "Agent still alive 100s after admin kill (watchdog failed)" \
...
# ================================================================
docker run -d --name s-a --network s-net --entrypoint /opt/shroud/admin shroud-test \
  --daemon -l 9999 -s fl_test --identity-plain --identity-dir /tmp 2>/dev/null; sleep 3
docker run -d --name s-g --network s-net --entrypoint /opt/shroud/agent shroud-test \
